"""
GETTING DADA   ->reporting.getdays()
ANALYSING THE DATA -> KEYWORDS
"""
import re
import requests
import sys
...
def saveFile(tic,score,betaList,tit,b):
        delimiter="`"
        f=open("dframe_new_line.txt","w").close()
        f=open("dframe_new_line.txt","a")
        for i in range(len(tit)):
            data=str(tic[i])+delimiter+str(score[i])+delimiter+str(betaList[i])\
                  +delimiter+str(tit[i])+delimiter+str(b[i])
...
def readFile(file="dframe_new_line.txt"):
        global pNotA
        global pA
        count1,count2,count3,count4=0,0,0,0
        tic,score,beta_list,tit,b=[],[],[],[],[]
        delimiter="`"
        f=open("dframe_new_line.txt","r")
        count=0
        for line in f:
            ...
            if line.count(delimiter)!=4:
                ...
                k=input()
                continue
            x,w,beta,y,z=line.split(delimiter)
            z=extraClean(z)
        ...
        return (tic,score,beta_list,tit,b)
...
def main(field_ticker_by_score,data_by_date=reporting):
        ...
        if k=='web':
            ...
            for ticker_date in field_ticker_by_score.keys():
                ...
                print(len(data.beta), type(data.beta))
                if ticker in data.beta.keys():
                        print(ticker in data.beta.keys())
                        print(type(data.beta[ticker]))
            ...
            saveFile(tic,score,beta_list,tit,b)
            keywords.go(tit,score,beta_list)
            return(tic,score,beta_list,tit,b)
            #k=input()
        else:
            print("Reading...")
            tic,score,beta_list,tit,b=readFile()
            keywords.go(tit,score,beta_list)
            return(tic,score,tit,b)
        print(len(tic),len(score),len(beta_list),len(tit),len(b))
        ...
        print(len(t2))
        """
#field_ticker_by_score,data_by_date=reporting.main()  # run this in the python command line to have access these variable later in case the code below or all this code break
