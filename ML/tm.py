...
def saveFile(tic,tit,b):
    delimiter="`"
    f=open("dframe_new_line.txt","w").close()
    f=open("dframe_new_line.txt","a")
    for i in range(len(tit)):
        data=str(tic[i])+delimiter+str(tit[i])+delimiter+str(b[i])
...
def readFile(file="dframe_new_line.txt"):
    ...
    for line in f:
        ...
        if not isinstance(z,str):
            ...
            k=input()
        #k=input()
        count1+=z.lower().count("income")
        count2+=z.lower().count("million")
        count3+=z.lower().count("offering")
        tic.append(x)
        tit.append(y)
        b.append(z)
        print(count,x,y)
        count+=1
    print(len(b))
    print("Count income-> ",count1)
    print("Count million-> ",count2)
    print("Count offering-> ",count3)
    k=input("Done")
