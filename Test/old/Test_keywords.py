...
def loadData(all_info):
        ...
        all_data=[]
        try: f=open('list_data.txt','r')
        except:
                f=open('list_data.txt','w').close()
        ...
        for line in data:
                if len(line)>0:
                        print(line)
                        if line.split()[1]==all_info[0]:
                                flag=False
                                all_data.append(time.strftime("%m-%d-%Y ")+" "+" ".join(all_info))
                        else: all_data.append(line)
        if flag==True:  all_data.append(time.strftime("%m-%d-%Y ")+" "+" ".join(all_info))
        f_data="\n".join(all_data)
...
#symbols_and_prices_file.write((ticker+" "+str(price)+" "+time.strftime("%Y%m.")+str((int(time.strftime("%d"))*24)+(int(time.strftime("%H"))))+" "+ticker_news[ticker]).replace("\n","")+'\n')
		...
		data=f.read()
		print("Lentgh of data -> ",len(data.split("\n")))
		#k=input()
		for each_data in data.split("\n"):
			...
			print("Keywords Found-> ",keywords_found)
			#k=input()
			loadData(all_info[:-1]+keywords_found+[all_info[-1]])
			#print(all_info)
