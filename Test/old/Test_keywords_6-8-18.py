...
def getAllInfo(ticker,new,old_price=1000):
	...
	return [str(ticker), str(volume), str(beta), str(opening), str(previous_close[0])+"-"+str(previous_close[1]),str(lowest_price), title.replace('\n',' ').replace('\r',' ')]
...
def update_old_records(data):
        ...
        for line in data:
                line_arr=line.split()
                if line_arr[8].find("http")>=0 and line_arr[0]==time.strftime("%m-%d-%Y") :# check if the ext file there is a url
                        #n= 8 #if line_arr[7].find("http")>=0 else 6
                        ticker=line_arr[1]
                        new=line_arr[8]
                        keywords_found=line_arr[7]
                        #keywords_list=line_arr[6]
                        #title=line_arr[8:]
                        old_price=line_arr[5].split("-")[0]
                        #k=input(old_price)
                        print(type(old_price))
                        print(old_price)
                        if old_price =='None': old_price=1000
                        else: old_price=float(old_price)
                        #k=input("============")
                        all_info=getAllInfo(ticker,new,old_price)
                        print("Old-> ",old_price," Returned-> ",all_info[5])
                       # k=input()
...
def loadData(all_info):
        ...
        all_data=[]
        try: f=open('list_data_2.txt','r')
        except:
                f=open('list_data_2.txt','w').close()
                f=open('list_data_2.txt','r')
