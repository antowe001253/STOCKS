...
def check_P_E(ticker,P_E):
    if P_E <0:
        result=get_info(ticker=ticker,info="PE Ratio",timeout=10)
...
def dayBefore():
        ...
        day=t_date.split("-")[-1]
        new_day=str(int(day)-1).zfill(2)
        print(new_day)
        t_date=t_date.split("-")[0]+"-"+t_date.split("-")[1]+"-"+new_day
        t_date=datetime.date(int(t_date.split("-")[0]), int(t_date.split("-")[1]), int(new_day))
        print(t_date)
        print(t_date==date.today())
        #k=input()
        return(t_date)
...
#days=getdays(6,2018)
while(1):
        days=getDatesUntilToday()
        f=open("date_cleanned.txt","r")
        date_used=f.read()
        f.close()
        #print(date_used)
        for each_day in days:
                if date_used.find(str(each_day))<0:
                        print("New date-> ",str(each_day))
                        #k=input()
                        main_(str(each_day))
                        f=open("date_cleanned.txt","a")
                        f.write(str(each_day)+'\n')
                        f.close()
                else:
                        print("Cleanned-> ",str(each_day))
                        continue
