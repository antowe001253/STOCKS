...
def write_in_file(nbr_emails,file_name,message):
	...
	f.write("Size of the list appending "+"["+str(nbr_emails)+"] "+"[TIME] "+time.strftime("%b %d, %Y---%H:%M:%S ")+message+'\n')
	f.close()
...
#k=input("===============")
def write_in_symbol_file_to_best_play_app(ticker):# the app best_play read in to that file to get the symbol and see if there is a change in price
        symbol_file=open("symbols.txt","a")
        symbol_file.write(ticker+'\n')
        symbol_file.close()
                
while(1):
    
    cheker = open("cheker.txt","w")
                    ...
                    if ((wesite.find("yahoo")>=0 or wesite.find("globenewswire")>=0 )and(g_txt_split_val.find("hours")<0 and g_txt_split_val.find("hour")<0 and (g_txt_split_val.find("minutes ago")>=0 or g_txt_split_val.find("minute ago")>=0))) or (wesite.find("marketwired")>=0 and g_txt_split_val.find("hours")<0 and g_txt_split_val.find("minutes ago")<0 and g_txt_split_val.find(time.strftime("%b %d, %Y"))>=0):
                       ...
                       #print(link_name)
                        ...
                        if title_name not in title_bank and len([ key for key in key_words if title_name.lower().find(key.lower())>=0])>0:
                            #print("Link Name -->",link_name)
                            if len(link_name)<10: k_input=input("link_name====================================================")
                           ...
                           # print("Whole link ->",whole_link_name)
                            ...
                            if (detail_page.status_code != 200):
                                ...
                                if title_name.lower().find(" date ")<0 and (detail_page.find('(<exchange')>=0 or globenewswire_iden) and(stock_ticker not in set(companies_abraviations)):
                                   ...
                                   # print('(<a -> ',detail_page.find('(<a')>=0,'\n(<exchange -> ',detail_page.find('(<exchange')>=0,'\n: <a href -> ',globenewswire_iden)
                                    ...
                                    if title_name.lower().find("available at")>=0:
                                       ...
                                       write_in_symbol_file_to_best_play_app(stock_ticker)
