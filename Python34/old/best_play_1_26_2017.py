...
#for ticker in list_of_symbols:
	try:
		...
		if detail_page.find("not found")>=0:
			...
			#print(detail_page)
			if detail_page.find(">Price</td")>=0:
				price_value=detail_page[detail_page.find(">Price</td"):]
				#print(type(price_value),price_value[:200])
				price_value = price_value[price_value.find("<b>")+3:300]
				#print(type(price_value),price_value)
				
				
				price_value = price_value[:price_value.find("</b>")]
			   # k=input(price_value)
			...
			#k=input("The requested symbol was not found in our database")
				...
				#price_value=detail_page[detail_page.find(">price</td"):]
				#price_value = price_value[price_value.find("<b>"+3):]
				#price_value = price_value[:price_value.find("</b>"]
				#k=input(price_value)
				#if len(price_value)<=0:
							#print(ticker," ====== NO PRICE FOUND  =====")
							#return get_price(ticker)
				#return float(detail_page)
...
def get_price_NASDAQ (ticker):
#for ticker in list_of_symbols:
	try:
		#g_page = requests.get("https://finance.yahoo.com/quote/"+ticker+"?p="+ticker)
		#g_page = requests.get("http://finviz.com/quote.ashx?t="+ticker)
		g_page = requests.get("http://www.nasdaq.com/symbol/"+ticker+"/real-time")
		detail_page=g_page.text
		count_script=detail_page.count("</script>")
									#k=input(count_script)
		#while(count_script>0):
		...
		##if detail_page.find("Symbols similar to")>=0 or detail_page.find("Try looking again!")>=0 :
		if detail_page.find("is an unknown symbol. You may")>=0 or detail_page.find("NASDAQ Official Price")<0:
			symbol_no_found_file=open("symbols_no_found.txt","a")
			symbol_no_found_file.write(ticker+'\n')
			print("http://www.nasdaq.com/symbol/"+ticker+"/real-time")
			print(g_page)
			print(ticker)
			print("is an unknown symbol. You may",detail_page.find("is an unknown symbol. You may"))
			print("NASDAQ Official Price",detail_page.find("NASDAQ Official Price"))
			k=input("The requested symbol was not found in our database RETURN -1")
			symbol_no_found_file.close()
			return -1
			
		else: 
			#print(detail_page)
			if detail_page.find("NASDAQ Official Price")>=0:
				#price_value=detail_page[detail_page.find("Common Stock Real Time Stock Quotes"):]
				print("IN->",ticker)
				#print(ticker)
				#print(ticker)
				price_value = detail_page[detail_page.find('qwidget-dollar">$'):]
				#print(price_value)
				price_value = price_value[price_value.find('">$')+3:]
				#price_value = price_value[:price_value.find(time.strftime("/%b. %d, %Y")]

				#print(price_value)
				
				price_value = price_value[:price_value.find("</div>")]
				...
				if len(price_value)<=0:
					print(ticker,price_value," ====== NO PRICE FOUND  =====")
					return get_price_NASDAQ(ticker)
				return float(price_value)
			...
			#    print(" === A F T E R   H O U R ===")#print("Found")
				...
				else: return(get_price_NASDAQ(ticker))
...
while(1):
		...
		for symbol, initial_price in scanner.items():
			...
			actual_price = get_price_YAHOO(symbol)
